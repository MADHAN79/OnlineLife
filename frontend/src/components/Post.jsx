...
const Post = ({ post }) => {
    ...
    const [liked, setLiked] = useState(post.likes.includes(user?._id) || false); // true || false(default value)
    ...
    const likeOrDislikeHandler = async () => {
        try {
            //if the post is already liked the dislike route is added else default like route is added below in ${action}
    ...
    const deletePostHandler = async () => {
        try {
            ...
            if (res.data.success) {
                ...
                //dispatch will automatically refreshes the feed so that the deleted post gets removed fromt the feed.
                dispatch(setPosts(updatedPostData));
                toast.success(res.data.message);
            }
        } catch (error) {
            ...
            <div className='flex items-center justify-between'>
                ...
                <Dialog>
                    ...
                    <DialogContent className="flex flex-col items-center text-sm text-center">
                        ...
                        {/* for the current user the unfollow button in the same current user's post not shown */}
                        {
                        post?.author?._id !== user?._id && <Button variant='ghost' className="cursor-pointer w-fit text-[#ED4956] font-bold">Unfollow</Button>
                        }
                        
                        {/* only for the logged-in author of the post delete button get showed */}

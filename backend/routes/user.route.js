...
router.route('/register').post(register); //eg: 'http://localhost:3000/api/v1/user/register' see the api route in index.js
router.route('/login').post(login);
router.route('/logout').get(logout);

//getProfile got accessed using the next() parameter in isAuthenticate.js, similarly all below.
router.route('/:id/profile').get(isAuthenticated, getProfile); //eg: http://localhost:3000/api/v1/user/66e46fc2e6bf3641ac8b380b/profile
